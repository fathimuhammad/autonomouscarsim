...
def callback(data):
    ...
    def perspectiveWarp(inpImage):
        ...
        minimum_distance = rospy.wait_for_message('/distanceEstimator/dist', Float64, timeout=1).data
        ...
        if current_time >= oldtime + 5:
            avoidance = 0
        #cv2.putText(img, "Obstacle Range : " + str(int(minimum_distance)), (10,150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (102,255,0), 2, cv2.LINE_AA)
        if len(contoursL)**2 >= 16 :
            #cv2.putText(img, "Position : Right Lane" , (10,120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (102,255,0), 2, cv2.LINE_AA)
            if avoidance == 1:
                steering_angle = 0.2
                obstacle()
            else:
            ...
            #cv2.putText(img, "Position : Left Lane" , (10,120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (102,255,0), 2, cv2.LINE_AA)
    ...
    speed = 6.75
    factor = 0.075
    speed = speed - abs(steering_angle) 
    if abs(deviation) >= 1.9:
        speed = speed - (abs(deviation)*4)
        factor = factor * 2
    else :
        speed = speed - abs(deviation)
    if speed <=3:
        speed =3    
    move(speed, steering_angle+((deviation*factor)))
    #cv2.putText(finalImg, "Speed : " + str(int(speed*10)) + " Km/H" , (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (102,255,0), 2, cv2.LINE_AA)
    print("Speed : " + str(int(speed*10)) + " Km/H")
